...
export const readFile = {
    name: "read_file",
    description: "读取指定文件的内容。当需要查看文件、理解代码、阅读文档时使用。支持文本文件。",
...
export const writeFile = {
    name: "write_file",
    description: "创建或覆盖写入文件。当需要保存内容、创建新文件、生成代码时使用。注意：这会覆盖已有文件。",
    parameters: Type.Object({
        filePath: Type.String({
...
export const listDirectory = {
    name: "list_directory",
    description: "列出目录下的文件和子目录。当需要了解项目结构、查找文件、浏览目录时使用。",
    parameters: Type.Object({
    ...
    async execute(_id, params, _signal) {
        try {
            ...
            return {
                content: [{ type: "text", text: `列目录失败: ${err.message}` }],
                details: { isError: true },
            };
        }
    },
...
export const webSearch = {
    name: "web_search",
    description: "联网搜索互联网信息。当需要查找最新资讯、技术文档、事实数据时使用。返回搜索结果的标题、摘要和链接。",
    parameters: Type.Object({
        query: Type.String({
...
async function duckDuckGoSearch(query) {
    ...
    try {
        ...
        const html = await fetchHtml(`https://lite.duckduckgo.com/lite/?q=${encoded}`, 10000);
    ...
    try {
        const encoded = encodeURIComponent(query);
        const html = await fetchHtml(`https://www.bing.com/search?q=${encoded}&setlang=zh-cn`, 10000);
...
export const getCurrentTime = {
    ...
    description: "获取当前日期和时间。当用户询问「现在几点」「今天几号」时使用。返回格式化时间字符串。",
    parameters: Type.Object({
...
export const calculator = {
    name: "calculator",
    description: "执行数学计算。支持加减乘除和括号。当用户要求计算数值时使用。",
    parameters: Type.Object({
